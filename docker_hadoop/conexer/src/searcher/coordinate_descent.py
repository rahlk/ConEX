...
class CoordinateDescent:
    ...
    def run(self):
        ...
        # init_conf = self.conf_space.get_default_conf()
        self.all_params = init_conf.keys()
        print '========= Default Configuration ==========='
        print init_conf
        init_perf = self.profile_with_type_checker(self.profile_num, init_conf)
        self.initial_perf = init_perf
    ...
    def converged(self):
        '''
        stopping criterion:
            1. achieve global performance improvement
            2. or achieve maximum iterations
        '''
        perf_improvement = (self.best_perf - self.initial_perf)*1.0/self.initial_perf
        if perf_improvement >= self.global_improvement:
            return True
        if self.profile_num >= self.max_iter:
            return True
        ...
        if param_data_type in [ConfDataType.integer, ConfDataType.float]:
            ...
            if curr_idx == 0:
                for v in all_values[1:]:
                    conf[param] = v
                    p = self.profile_with_type_checker(self.profile_num, conf)
            ...
            # if the current value is the last one of the value list
                ...
                previous_perf = self.profile_with_type_checker(self.profile_num, conf)
                print 'Iteration:', self.profile_num, 'Parameter:', param, '=== Value:', \
                ...
                conf[param] = all_values[curr_idx + 1]
                next_perf = self.profile_with_type_checker(self.profile_num, conf)
                print 'Iteration:', self.profile_num, 'Parameter:', param, '=== Value:', \
    ...
    def profile_all_values(self, param, values, init_conf, perf_dict):
        ...
        for v in values:
            new_conf[param] = v
            perf = self.profile_with_type_checker(self.profile_num, new_conf)
