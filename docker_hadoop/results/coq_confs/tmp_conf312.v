Require Import hadoop_config.
    ...
    unshelve refine (
        mk_core_config
        (io_file_buffer_size.mk false 53108%positive _ )
...
Definition a_hdfs_config: HDFSConfig.
    ...
    (dfs_replication.mk false 3%positive _ )
); try (exact I).
    Defined.
...
Definition a_mapred_config: MapRedConfig.
    ...
    (mapreduce_input_fileinputformat_split_minsize.mk false 33554432%N _ )
    (mapreduce_input_lineinputformat_linespermap.mk false 1%positive _ )
    (mapreduce_job_counters_max.mk false 120%positive _ )
    (mapreduce_job_jvm_numtasks.mk false (Some 3%positive) _ )
    ...
    (mapreduce_map_memory_mb.mk false 6144%positive _ )
    (mapreduce_map_output_compress.mk false false _ )
    (mapreduce_map_output_compress_codec.mk false "org.apache.hadoop.io.compress.GzipCodec" _ )
    ...
    (mapreduce_reduce_input_buffer_percent.mk false (10/100)%R _ )
    (mapreduce_reduce_java_opts.mk false (mk_java_opts 1312%positive 100%positive) _ )
    (mapreduce_reduce_markreset_buffer_percent.mk false (0/100)%R _ )
    (mapreduce_reduce_memory_mb.mk false 4096%positive _ )
    (mapreduce_reduce_merge_inmem_threshold.mk false (Some 900%positive) _ )
    ...
    (mapreduce_task_io_sort_mb.mk false 90%positive _ )
    ...
    (mapreduce_tasktracker_map_tasks_maximum.mk false 4%positive _ )
    (mapreduce_tasktracker_reduce_tasks_maximum.mk false 4%positive _ )
    ...
    (yarn_app_mapreduce_am_resource_cpu__vcores.mk false 2%positive _ )
    (yarn_app_mapreduce_am_resource_mb.mk false 2300%positive _ )
