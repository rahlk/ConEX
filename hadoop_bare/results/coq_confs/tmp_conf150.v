Require Import hadoop_config.
    ...
    unshelve refine (
        mk_core_config
        (io_file_buffer_size.mk false 49152%positive _ )
    (io_map_index_interval.mk false 128%positive _ )
    (io_map_index_skip.mk false 0%N _ )
    (io_seqfile_compress_blocksize.mk false 1100000%positive _ )
    (io_seqfile_sorter_recordlimit.mk false 900000%positive _ )
...
Definition a_mapred_config: MapRedConfig.
    ...
    (mapreduce_job_jvm_numtasks.mk false (Some 2%positive) _ )
    (mapreduce_job_max_split_locations.mk false 12%positive _ )
    (mapreduce_job_reduce_slowstart_completedmaps.mk false (8/100)%R _ )
    ...
    (mapreduce_job_ubertask_maxmaps.mk false 5%positive _ )
    (mapreduce_job_ubertask_maxreduces.mk false (Some 6%positive) _ )
    ...
    (mapreduce_map_cpu_vcores.mk false 2%positive _ )
    (mapreduce_map_java_opts.mk false (mk_java_opts 737%positive 100%positive) _ )
    (mapreduce_map_memory_mb.mk false 5120%positive _ )
    ...
    (mapreduce_output_fileoutputformat_compress_codec.mk false "org.apache.hadoop.io.compress.BZip2Codec" _ )
    (mapreduce_output_fileoutputformat_compress_type.mk false "BLOCK" _ )
    (mapreduce_reduce_cpu_vcores.mk false 3%positive _ )
    (mapreduce_reduce_input_buffer_percent.mk false (20/100)%R _ )
    ...
    (mapreduce_reduce_memory_mb.mk false 4096%positive _ )
    (mapreduce_reduce_merge_inmem_threshold.mk false (Some 1200%positive) _ )
    (mapreduce_reduce_shuffle_input_buffer_percent.mk false (60/100)%R _ )
    (mapreduce_reduce_shuffle_memory_limit_percent.mk false (15/100)%R _ )
    (mapreduce_reduce_shuffle_merge_percent.mk false (80/100)%R _ )
    (mapreduce_reduce_shuffle_parallelcopies.mk false 9%positive _ )
    (mapreduce_shuffle_max_connections.mk false (Some 2%positive) _ )
    ...
    (mapreduce_task_io_sort_mb.mk false 110%positive _ )
    ...
    (mapreduce_tasktracker_http_threads.mk false 60%positive _ )
    (mapreduce_tasktracker_indexcache_mb.mk false 10%positive _ )
    (mapreduce_tasktracker_map_tasks_maximum.mk false 5%positive _ )
    ...
    (yarn_app_mapreduce_am_command__opts.mk false (mk_java_opts 800%positive 100%positive) _ )
