...
docker cp config/hadoop.conf    hadoop-master:/root/HiBench/conf/
...
do 
  ...
  docker exec -it hadoop-master /root/HiBench/bin/run_all.sh > /dev/null
  cat /root/HiBench/report/wordcount/hadoopbench.log | grep 'CPU time spent' | sed 's/\t//g' | tee scale_up_runtimes.txt
done 
